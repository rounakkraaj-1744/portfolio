    ...
    <section id="work" className="py-16 sm:py-20">
      <div className="max-w-[1120px] mx-auto px-6 sm:px-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
          ...
          <Link href="https://github.com/rounakkraaj-1744" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full border border-[#D5D3CC] bg-white/60 text-[#111111] text-xs font-medium hover:bg-white transition-all shrink-0 self-start sm:self-auto" >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {featuredProjects.map((project) => (
            <div key={project.title} className="group bg-white rounded-2xl p-3.5 border border-[#E7E5DE] flex flex-col justify-between" >
              ...
              <div className="relative w-full aspect-[16/9.6] rounded-xl overflow-hidden bg-[#181C24] border border-black/5 mb-3.5">
                <Image src={project.image} alt={project.title} fill className="object-cover group-hover:scale-102 transition-transform duration-300"/>
              </div>

              <h3 className="text-sm sm:text-[15px] font-bold text-[#111111] tracking-tight">
                {project.title}
              </h3>
              ...
              <div className="flex items-center justify-between gap-2 mt-4">
                <div className="flex items-center gap-1.5 flex-wrap">
                  {project.tags.map((tag) => (
                    <span key={tag} className="px-2 py-0.5 rounded-md bg-[#F2F0E8] text-[#555] text-[10px] font-medium font-mono">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <Link href={project.link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 mt-3 text-[11px] font-semibold text-[#111] hover:text-black transition-colors">
